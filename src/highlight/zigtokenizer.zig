...
fn idx(buffer: []const u8, i: usize) u8 {
    return if (i >= buffer.len) 0 else buffer[i];
}
...
pub const Tokenizer = struct {
    ...
    pub fn next(self: *Tokenizer) Token {
        ...
        state: switch (State.start) {
            .start => switch (idx(self.buffer, self.index)) {
                0 => {
            ...
            .expect_newline => {
                self.index += 1;
                switch (idx(self.buffer, self.index)) {
                    0 => {
                        if (self.index == self.buffer.len) {
                            result.tag = .invalid;
                        } else {
            ...
            .multiline_string_literal_line => {
                ...
                switch (idx(self.buffer, self.index)) {
                    ...
                    '\n' => {},
                    '\r' => if (idx(self.buffer, self.index + 1) != '\n') {
                        continue :state .invalid;
                    },
            ...
            .doc_comment_start => {
                ...
                switch (idx(self.buffer, self.index)) {
                    ...
                    '\r' => {
                        if (idx(self.buffer, self.index + 1) == '\n') {
                            result.tag = .doc_comment;
                        } else {
                            continue :state .invalid;
                        }
            ...
            .int => switch (idx(self.buffer, self.index)) {
            ...
            .float => switch (idx(self.buffer, self.index)) {
                '_', 'a'...'d', 'f'...'o', 'q'...'z', 'A'...'D', 'F'...'O', 'Q'...'Z', '0'...'9' => {
                    self.index += 1;
                    continue :state .float;
                },
