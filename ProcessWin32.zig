...
pub const Io = struct {
    ...
    pub fn init(
        ...
        // TODO: is this right?
        switch (kind) {
            .pipe => try setInherit(out_err, stderr_pipe.read),
            .console => {},
        }
...
// fn setNonBlocking(out_err: *Win32Error, handle: win32.HANDLE) error{Error}!void {
//     var mode: win32.NAMED_PIPE_MODE = .{ .NOWAIT = 1 };
//     if (0 == win32.SetNamedPipeHandleState(handle, &mode, null, null)) {
//         return out_err.setWin32("SetNamedPipeHandleState", win32.GetLastError());
//     }
// }
fn setInherit(out_err: *Error, handle: win32.HANDLE) error{Error}!void {
